...
if __name__ == '__main__':
    ...
    title = md(str(title[0]))
    # auto.mkreadme(title.strip('\n'))

    description = auto.parsing("#problem_description") # 문제설명
    description = md(str(description[0]))
    # auto.mkreadme(description.strip('\n'))

    ex_input = auto.parsing("#problem_input > p") # 입력        
    ex_input = md(str(ex_input[0]))    
    # auto.mkreadme(ex_input.strip('\n'))

    ex_output = auto.parsing("#problem_output") # 출력
    ex_output = md(str(ex_output[0]))
    # auto.mkreadme(ex_output.strip('\n'))
